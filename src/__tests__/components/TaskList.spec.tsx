...
describe('App Page', () => {
  it('should be able to add a task', async () => {
    ...
    fireEvent.change(taskInput, {
      target: {
        value: 'Desafio Comeia'
      }
    });
    fireEvent.click(addTaskButton);

    const addedFirstTaskTitle = screen.getByText('Desafio Comeia');

    expect(addedFirstTaskTitle).toHaveTextContent('Desafio Comeia');
    ...
    const addedSecondTaskTitle = screen.getByText('Estudar');

    expect(addedFirstTaskTitle).toBeInTheDocument();
    expect(addedFirstTaskTitle).toHaveTextContent('Desafio Comeia');
    expect(addedFirstTaskTitle.parentElement).not.toHaveClass('completed')

    expect(addedSecondTaskTitle).toHaveTextContent('Estudar');
